...
import { createSkillVerification, updateProfile, fetchGitHubStats } from '@/services/firestore';
import { analyzeGitHubProfile, extractGitHubUsername } from '@/services/githubService';
...
const SkillVerificationModal = ({ userId, userName, userSkills, onClose, onComplete }: SkillVerificationModalProps) => {
  const { user } = useAuth();
  ...
  const handleGitHubConnect = async () => {
    ...
    const urlUsername = extractGitHubUsernameFromUrl(githubUrl);
    if (!urlUsername) {
      setGithubError('Could not extract username from URL');
      return;
    }

    if (!user || !auth.currentUser) {
      setGithubError('You must be logged in to verify your GitHub');
      return;
    }
    ...
    if (currentUser.uid !== originalUid) {
      setGithubError('Session mismatch detected. Please refresh and try again.');
      return;
    }
    ...
    try {
      ...
      const hasGithubLinked = currentUser.providerData.some(p => p.providerId === 'github.com');
      ...
      const analysis = await analyzeGitHubProfile(authenticatedUsername);

      // Match skills with user's profile skills
      const allInferredSkills = new Set<string>();
      analysis.inferredSkills.forEach(skill => allInferredSkills.add(skill.toLowerCase()));
      ...
      if (verifiedSkills.length === 0) {
        setGithubError(
          'None of your profile skills could be verified from your GitHub repositories. ' +
          'Please ensure your repositories contain code matching your listed skills.'
        );
        setGithubStep('input');
        setGithubLoading(false);
        return;
      ...
      await createSkillVerification(userId, {
        status: 'verified',
        verifiedSkills,
        sources: {
          github: {
            username: authenticatedUsername,
            profileUrl: `https://github.com/${authenticatedUsername}`,
            oauthVerified: true,
      ...
      toast.success(`Successfully verified ${verifiedSkills.length} skill(s) via GitHub!`);
      ...
      if (err.message?.includes('SECURITY_VIOLATION')) {
        await auth.signOut();
        setGithubError('Security issue detected. You have been signed out for safety. Please sign in again.');
        setGithubStep('input');
        setGithubLoading(false);
        return;
      }
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        setGithubError('Authentication cancelled. Please try again.');
        setGithubStep('input');
        setGithubLoading(false);
        return;
      }
      ...
      if (err.code === 'auth/credential-already-in-use') {
        setGithubError(
          'This GitHub account is already linked to another user. Each GitHub account can only be verified by one user.'
        );
        setGithubStep('input');
        setGithubLoading(false);
        return;
      ...
      if (err.code === 'auth/email-already-in-use') {
        setGithubError(
          `This GitHub account's email is already registered. Use a different GitHub account or contact support.`
        );
        setGithubStep('input');
        setGithubLoading(false);
        return;
      ...
      if (err.code === 'auth/provider-already-linked') {
        setGithubError('GitHub is already linked. Please try again.');
        setGithubStep('input');
        setGithubLoading(false);
        return;
      }
      ...
      setGithubError(err.message || 'GitHub verification failed. Please try again.');
      setGithubStep('input');
    } finally {
      setGithubLoading(false);
    }
