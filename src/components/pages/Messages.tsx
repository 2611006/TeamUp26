...
const Messages = ({ initialConversationId, onBack, onViewProfile }: MessagesProps) => {
  const { user } = useAuth();
  ...
  onClick={() => {
    const otherId = getOtherParticipant(selectedConv).id;
    onViewProfile?.(otherId);
  }}
  className="w-10 h-10 rounded-full object-cover cursor-pointer hover:ring-2 hover:ring-primary"
/>
